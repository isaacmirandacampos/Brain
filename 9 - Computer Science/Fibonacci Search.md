...
**Keywords:** #mathematics #algorithm #performance
...
def fibonacciSearch(arr, x):
	# Inicializa os três números Fibonacci
	fibMMm2 = 0 # (m-2)º número Fibonacci
	fibMMm1 = 1 # (m-1)º número Fibonacci
	fibM = fibMMm2 + fibMMm1 # mº número Fibonacci
	# fibM está indo armazenar o menor número Fibonacci
	# maior ou igual ao tamanho de arr
	while (fibM < len(arr)):
		fibMMm2 = fibMMm1
		fibMMm1 = fibM
		fibM = fibMMm2 + fibMMm1
	# Marca a faixa eliminada do array frontal
	offset = -1
	# Enquanto há elementos para serem inspecionados
	# Note que estamos cortando o array em duas partes,
	# fibMMm2 e fibMMm1, definindo o tamanho dessas partes
	while (fibM > 1):
		# Checa se fibMMm2 é uma localização válida
		i = min(offset + fibMMm2, len(arr) - 1)
		# Se x for maior que o valor no índice fibMMm2,
		# corta o subarray do array após i+1
		if (arr[i] < x):
			fibM = fibMMm1
			fibMMm1 = fibMMm2
			fibMMm2 = fibM - fibMMm1
			offset = i
			# Se x for menor que o valor no índice fibMMm2,
			# corta o subarray antes de i
		elif (arr[i] > x):
			fibM = fibMMm2
